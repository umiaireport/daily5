...
type CandidatePack = readonly CompiledDailyCase[];
type MatchableInput = readonly CompiledDailyCase[] | readonly CandidatePack[];

function isNested(value: MatchableInput): value is readonly CandidatePack[] {
  return value.length > 0 && Array.isArray(value[0]);
}

function assetKey(value: CompiledDailyCase): string {
...
function comparable(
  left: CompiledDailyCase,
  right: CompiledDailyCase,
...
function normalizeFlat(
  cases: readonly CompiledDailyCase[],
  expectedRounds: number,
  rejected: VariantRejection[],
): CandidatePack[] {
  const byRound = new Map<number, CompiledDailyCase[]>();
  const ids = new Set<string>();
