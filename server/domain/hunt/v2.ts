...
function resultFor(
  ...
  let explanation = '';
  if (reason === 'caught')
    explanation = `The tracer locked ${selectedZone}; buying accumulated in ${hiddenZone}.`;
  else if (reason === 'escaped')
    explanation = `The large pulse in ${decoyZone ?? otherZone(hiddenZone)} was a decoy. The real trade accumulated in ${hiddenZone}.`;
    ...
    explanation = `The tracer ran out of time. The whale was hidden in ${hiddenZone}.`;
...
function validateStateVersion(row: HuntV2MatchRow, expected: number): void {
  if (row.state_version !== expected)
    throw new HuntV2Error(
      'STALE_STATE',
      'The Hunt state changed. Refresh the arena and try again.',
      409,
      true,
      row.state_version,
    );
...
function advanceExpired(row: HuntV2MatchRow, now: Date, boardFactory: HuntV2BoardFactory): boolean {
  ...
  if (row.phase === 'round_reveal') {
    if (row.score_whale >= 3 || row.score_tracer >= 3) {
...
function scanResult(
  ...
  const marketSignal = round.windows.find((window) => window.zone === zone)?.market?.smartMoney;
  const signal =
...
export class HuntV2Engine {
  ...
  private view(row: HuntV2MatchRow, actorId: string, now: Date): HuntV2MatchView {
    ...
    const winner = row.score_whale >= 3 ? 'whale' : row.score_tracer >= 3 ? 'tracer' : null;
