...
test('five-round expedition, clue budget, reload, benchmarks, share and leaderboard', async ({
  ...
  await page.screenshot({
    path: `screenshots/${capturePrefix}viewport-${testInfo.project.name}.png`,
  });
  await page.screenshot({
    path: `screenshots/${capturePrefix}arena-${testInfo.project.name}.png`,
    fullPage: true,
  });
  ...
  for (let round = 1; round <= 5; round++) {
    ...
    if (round === 1) {
      ...
      await download.saveAs(`screenshots/${capturePrefix}scorecard-${testInfo.project.name}.png`);
      await page.screenshot({
        path: `screenshots/${capturePrefix}reveal-${testInfo.project.name}.png`,
        fullPage: true,
      });
    }
  ...
  await page.screenshot({
    path: `screenshots/${capturePrefix}complete-${testInfo.project.name}.png`,
    fullPage: true,
  });
