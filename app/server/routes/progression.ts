...
const comparisonQuery = z
  .object({
    scope: z.enum(['exact-variant', 'cohort']).optional(),
    variantId: z.string().min(1).max(200).optional(),
    cohortId: z.string().min(1).max(200).optional(),
  })
  .strict();
...
/** Registers the profile, honest Daily comparison, and public sanitized share boundaries. */
export async function registerProgressionRoutes(
  ...
  app.get('/api/daily-five/:id/comparison', async (request, reply) =>
    safely(reply, () => {
      const { id } = idSchema.parse(request.params);
      const query = comparisonQuery.parse(request.query);
      return options.service.compareDaily({
        playerId: identity(request, options),
        dailyId: id,
        scope: query.scope,
